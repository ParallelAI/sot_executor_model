...
object Topology {
  ...
  def buildVertices[V, E <: Edge[V]](v: List[V], top: Topology[V, E]): Topology[V, E] = {
    v match {
      case h :: t => buildVertices(t, top.addVertex(h))
      case nill => top
    }
  }

  def buildEdges[V, E <: Edge[V]](e: List[E], top: Topology[V, E]): Topology[V, E] = {
    e match {
      case h :: t => {
        val Success((newTop, _)) = top.addEdge(h)
        buildEdges[V, E](t, newTop)
      }
      case nill => top
    }
  }

  def createTopology[V, E <: Edge[V]](v: List[V], e: List[E]) = {
    val top = Topology.empty[V, E]
    val topV = buildVertices(v, top)
    buildEdges(e, topV)
  }
...
final case class Topology[V, E <: Topology.Edge[V]] private(vertices: Vec[V], edges: Set[E])
  ...
  private def discovery(visited: MSet[V], newEdgeMap: Map[V, Set[E]], v: V, upBound: Int): Boolean = {
    ...
    while (targets.nonEmpty) {
      val v = targets.pop()
      visited += v
      val moreTargets = newEdgeMap.getOrElse(v, Set.empty).map(_.to)
  ...
  // find sink nodes
  // brute-force approach, can be improved if this is a bottleneck
  def getSinkVertices() = {
  ...
  //Returns vertices with more than n incoming edges
  ...
  //Returns vertices with more than n outgoing edges
