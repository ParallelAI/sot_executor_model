...
class JSONDefinitionsSpec extends WordSpec with Matchers {
  ...
  "SOTMacroConfig" should {
    ...
    "parse pubsub to datastore json string" in {
      ...
      val dag = List(
        DAGMapping(from = "source1", to = "filter"),
        DAGMapping(from = "filter", to = "mapper1"),
        DAGMapping(from = "mapper1", to = "sumByKey"),
        DAGMapping(from = "sumByKey", to = "mapper2"),
        DAGMapping(from = "mapper2", to = "sink1")
      )
      val steps = List(
        Op(`type` = "source", name = "source1", op = "", func = ""),
        Op(`type` = "transformation", name = "mapper1", op = "map", func = "m => (m.teamName, m.score.toInt)"),
        ...
        Op(`type` = "sink", name =  "sink1", op = "", func = "")
      )
